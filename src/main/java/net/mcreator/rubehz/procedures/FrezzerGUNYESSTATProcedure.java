...
public class FrezzerGUNYESSTATProcedure {
	public static void execute(Entity entity) {
		...
		if ((entity.getCapability(RubehzModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RubehzModVariables.PlayerVariables())).intelligence > 9
			...
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(RubehzModItems.FREZZER_GUNNOSTAT.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(RubehzModItems.FREZZER_GUN.get()).copy();
